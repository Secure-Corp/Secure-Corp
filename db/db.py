...
#Empezamos a crear todas las tablas junto con los datos de nuestra base de datos y en caso de encontrrar un error lo mostrara en consola
    ...
    def crearTablaEmpleado(self):
        try:
            self.cursor.execute("CREATE TABLE if not exists empleado (idEmpleado int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, idRequisicion int(11) NOT NULL, idPuesto int(11) NOT NULL, CURP varchar(30) NOT NULL, RFC varchar(20) NOT NULL, nombre varchar(40) NOT NULL, descripcion varchar(255) DEFAULT NULL, apellido varchar(40) NOT NULL, domCalle varchar(40) NOT NULL, domNumExtInt varchar(30) NOT NULL, domColonia varchar(40) NOT NULL, tel1 varchar(20) NOT NULL, sueldo varchar(20) NOT NULL, correoE varchar(40) NOT NULL, edad int(11) NOT NULL, sexo varchar(10) NOT NULL, idEstadoCivil int(11) NOT NULL, idEscolaridad int(11) NOT NULL, idGradoAvance int(11) NOT NULL, idCarrera int(11) NOT NULL)")
            self.cursor.execute("SELECT COUNT(*) FROM empleado")
            count = self.cursor.fetchone()[0]
            if count == 0:
                self.cursor.execute("INSERT INTO empleado (idRequisicion, idPuesto, CURP, RFC, nombre, apellido, domCalle, domNumExtInt, domColonia, tel1, sueldo, correoE, edad, sexo, idEstadoCivil, idEscolaridad, idGradoAvance, idCarrera) VALUES (1, 1, 'ROGH760106MASDML03', 'dfadf', 'Carlitos', 'Muntez', 'Villas', '#23', 'palmas', '4491102343', '$233', 'carlitos@gmail.com', 23, 'Macho', 1, 2, 1, 1)")
                self.cursor.execute("INSERT INTO empleado (idRequisicion, idPuesto, CURP, RFC, nombre, apellido, domCalle, domNumExtInt, domColonia, tel1, sueldo, correoE, edad, sexo, idEstadoCivil, idEscolaridad, idGradoAvance, idCarrera) VALUES (1, 1, 'ROML551119HASDCR08', 'dfajy', 'Pepe', 'Maciado', 'Potreros', '#26', 'cruz', '4491739435', '$2223', 'pepe@gmail.com', 23, 'Indistinto', 3, 2, 3, 1)")
        except pymysql.Error as err:
